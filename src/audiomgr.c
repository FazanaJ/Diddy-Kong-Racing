...
void amCreateAudioMgr(ALSynConfig *c, OSPri pri, OSSched *audSched) {
    ...
    if (c->fxType[0] == AL_FX_CUSTOM) {
        ...
        asset8 = mempool_alloc_audio(gAudioPool, assetSize, MEMP_AUDIO);
    ...
    asset = mempool_alloc_safe((maxFrameSize * 12), MEMP_AUDIO_POOL);
    ...
    osCreateThread(&__am.thread, 4, __amMain, 0, (void *) (audioStack + THREAD4_STACK / sizeof(u64)), pri);
}
