...
s32 D_8012A0E8[64];
s16 *gWaveBlockIDs; // used to index gWaveModel and as arg0 for func_800B92F4 and func_800B97A8
...
void waves_alloc(void) {
    ...
    waves_free();
    gWaveBlockIDs = (s16 *) mempool_alloc_safe(512 * sizeof(s16), PP_RAM_WAVES);
    gWaveHeightTable = (f32 *) mempool_alloc_safe(gWaveController.seedSize * sizeof(f32), PP_RAM_WAVES);
    gWaveHeightIndices = (Vec2s *) mempool_alloc_safe(
        (gWaveController.tileCount * sizeof(Vec2s *)) * gWaveController.tileCount, PP_RAM_WAVES);
    ...
    if (gWavePlayerCount != 2) { // 1 player
        gWaveVertices[0][0] = (Vertex *) mempool_alloc_safe(allocSize << 1, PP_RAM_WAVES);
        gWaveVertices[1][0] = (Vertex *) (((u32) gWaveVertices[0][0]) + allocSize);
    } else { // 2 Player
        gWaveVertices[0][0] = (Vertex *) mempool_alloc_safe(allocSize << 2, PP_RAM_WAVES);
        gWaveVertices[1][0] = (Vertex *) (((u32) gWaveVertices[0][0]) + allocSize);
    ...
    if (gWavePlayerCount != 2) { // 1 Player
        gWaveTriangles[0][0] = mempool_alloc_safe(allocSize << 1, PP_RAM_WAVES);
        gWaveTriangles[1][0] = (Triangle *) (((u32) gWaveTriangles[0][0]) + allocSize);
    } else { // 2 Player
        gWaveTriangles[0][0] = (Triangle *) mempool_alloc_safe(allocSize << 2, PP_RAM_WAVES);
        gWaveTriangles[1][0] = (Triangle *) (((u32) gWaveTriangles[0][0]) + allocSize);
...
void func_800BBF78(LevelModel *model) {
    ...
    D_800E30D4 = mempool_alloc_safe(gWaveTileCountX * gWaveTileCountZ * sizeof(uintptr_t), PP_RAM_WAVES);
...
void func_800BCC70(LevelModel *model) {
    ...
    D_800E3178 = mempool_alloc_safe(model->numberOfSegments * gWaveTileTotal, PP_RAM_WAVES);
    spA0 = mempool_alloc_safe(model->numberOfSegments * 4, PP_RAM_WAVES);
    spA4 = mempool_alloc_safe((gWaveTileCountX * gWaveTileCountZ) * 8, PP_RAM_WAVES);
...
Object_Log *obj_wave_init(s32 blockID, f32 x, f32 z) {
    ...
    if (gWaveHeightTable != NULL && blockID >= 0 && blockID < gNumberOfLevelSegments) {
        object = mempool_alloc_safe((gWaveController.seedSize >> 1) + (sizeof(Object_Log) - 2), PP_RAM_WAVES);
