...
void obj_loop_fireball_octoweapon(Object *obj, s32 updateRate) {
    ...
    if (obj->behaviorId == BHV_FIREBALL_OCTOWEAPON_2 && obj->properties.fireball.timer < 0) {
        ...
        diff = (someObj->segment.trans.x_position - obj->segment.trans.x_position) * 0.1f;
        if (diff > 10.0f) {
            diff = 10.0f;
        }
        if (diff < -10.0f) {
        ...
        diff = (someObj->segment.trans.y_position - obj->segment.trans.y_position) * 0.1f;
        if (diff > 10.0f) {
            diff = 10.0f;
        }
        if (diff < -10.0f) {
        ...
        diff = (someObj->segment.trans.z_position - obj->segment.trans.z_position) * 0.1f;
        if (diff > 10.0f) {
            diff = 10.0f;
        }
        if (diff < -10.0f) {
...
void obj_loop_laserbolt(Object *obj, s32 updateRate) {
    ...
    move_object(obj, obj->segment.x_velocity * updateRateF, obj->segment.y_velocity * updateRateF, obj->segment.z_velocity * updateRateF);
    if (hasCollision) {
        func_8003FC44(obj->segment.trans.x_position, obj->segment.trans.y_position - 36.0f, obj->segment.trans.z_position, 44, SOUND_NONE, 0.2f, 0);
        delete = TRUE;
    }
    ...
    if (obj->interactObj->distance < 80) {
        ...
        if (racerObj && racerObj->behaviorId == BHV_RACER) {
            ...
            delete = TRUE;
            func_8003FC44(obj->segment.trans.x_position, obj->segment.trans.y_position - 36.0f, obj->segment.trans.z_position, BHV_LENS_FLARE_SWITCH, SOUND_EXPLOSION, 0.5f, 0);
        }
    }
    if (delete) {
        free_object(obj);
...
void obj_loop_collectegg(Object *obj, s32 updateRate) {
    ...
    case EGG_MOVING:
        ...
        if (obj->segment.trans.y_position < -2000.0f || (hasCollision && (surface < SURFACE_EGG_SPAWN || surface > SURFACE_EGG_04))) {
    ...
    if (obj->interactObj->distance < obj->properties.zipper.radius) {
        ...
        for (i = 0; i < numObjects; i++) {
            ...
            if (racer->boostTimer < 15 && racer->groundedWheels) {
                ...
                if ((s32) ((diffX * diffX) + (diffY * diffY) + (diffZ * diffZ)) < obj->properties.zipper.radius * obj->properties.zipper.radius) {
                    if (racer->playerIndex != PLAYER_COMPUTER) {
                        ...
                        play_random_character_voice(racerObj, SOUND_VOICE_CHARACTER_POSITIVE, 8, 0x82);
                    }
                    racer->boostTimer = (45);
...
void obj_loop_stopwatchman(Object *obj, s32 updateRate) {
    ...
    if (obj->properties.npc.action == TT_MODE_ROAM && distance < 300.0f && obj->properties.npc.timer == 0) {
    ...
    case TT_MODE_APPROACH_PLAYER:
        ...
        if (distance > 10.0f) {
            ...
            tt->forwardVel += (distance - tt->forwardVel) * 0.125f;
    ...
    default:
        ...
        if (tt->unkD == 0xFF) {
            ...
            tt->animFrameF += diffZ * 1.5f;
        }
        break;
    }
    obj->segment.trans.y_position = tempPosY;
    index = func_8002B0F4(obj->segment.object.segmentID, obj->segment.trans.x_position, obj->segment.trans.z_position, &water);
     if(index != 0) {
        index--;
        while(index >= 0) {
            if ((water[index]->type != WATER_CALM) && (water[index]->type != WATER_WAVY) && (water[index]->rotY > 0.0f)) {
    ...
    obj->segment.animFrame = 1.0f * tt->animFrameF;
    func_80061C0C(obj);
 ...
 * Count down until zero, then play an animation, after which, set the down time to a random amount with an offset based on spawn properties.
*/
void obj_loop_lavaspurt(Object *obj, s32 updateRate) {
    if (obj->properties.lavaSpurt.actionTimer > 0) {
        ...
        if (obj->segment.animFrame > 255) {
            ...
            obj->properties.lavaSpurt.actionTimer = get_random_number_from_range(0, 30) + obj->properties.lavaSpurt.delayTimer;
        }
    }
}
...
void obj_loop_parkwarden(Object *obj, s32 updateRate) {
    ...
    var_a2 = FALSE;
    if (
        (obj->properties.npc.action == NULL) &&
        (distance < 300.0f) &&
        (
    ...
    case TAJ_MODE_APPROACH_PLAYER:
        ...
        if (distance > 10.0f) {
            ...
            taj->forwardVel += (var_f2 - taj->forwardVel) * 0.125f;
            ...
            taj->animFrameF -= taj->forwardVel * 2.0f * updateRateF;
        } else {
    ...
    case TAJ_MODE_GREET_PLAYER:
        ...
        if (taj->animFrameF > 77.0f) {
            taj->animFrameF = 77.0f;
            taj->unk18 = -1.0f;
            obj->properties.npc.action = TAJ_MODE_DIALOGUE;
        }
        arctan = (racerObj->segment.trans.y_rotation - (obj->segment.trans.y_rotation & 0xFFFF)) + 0x8000;
    ...
    case TAJ_MODE_TRANSFORM_BEGIN:
        ...
        if (taj->animFrameF > 50.0f) {
            obj->unk74 = 0;
        }
        if (taj->animFrameF > 60.0f) {
    ...
    case TAJ_MODE_END_DIALOGUE_UNUSED:
        if (taj->animFrameF != 0.0f) {
            taj->animFrameF = taj->animFrameF + (0.5f * updateRateF);
        }
        if (taj->animFrameF == 0) {
            sp6B = 1;
            if (obj->properties.npc.action == TAJ_MODE_END_DIALOGUE_UNUSED) {
                func_80022CFC(
                    obj->segment.object.segmentID,
                    obj->segment.trans.x_position - (racer->ox1 * 50.0f),
                    obj->segment.trans.y_position,
                    obj->segment.trans.z_position - (racer->oz1 * 50.0f)
                );
            }
    ...
    default:
        ...
        if (taj->unkD == 0xFF) {
            ...
            if (taj->unk1C < 120) {
                ...
                move_object(obj, (updateRateF2 * xPosDiff) * 1.1f, 0.0f, (updateRateF2 * zPosDiff) * 1.1f);
                taj->animFrameF += updateRate * 2.2f;
            }
        }
    ...
    obj->segment.trans.y_position = tempPosY;
    var_a2 = func_8002B0F4(obj->segment.object.segmentID, obj->segment.trans.x_position, obj->segment.trans.z_position, &water);
    if(var_a2 != 0) {
        var_a2--;
        while(var_a2 >= 0) {
            if ((water[var_a2]->type != WATER_CALM) && (water[var_a2]->type != WATER_WAVY) && (water[var_a2]->rotY > 0.0f)) {
    ...
    obj->segment.animFrame = taj->animFrameF * 1.0f;
    func_80061C0C(obj);
    func_800AFC3C(obj, updateRate);
}
...
void obj_loop_door(Object *doorObj, s32 updateRate) {
    ...
    if (door->unkE >= 0) {
        ...
        if (door->unkF & 2) {
            ...
            if (door->unk14[1] == 1) {
                if (doorObj->segment.trans.y_position < (door->homeY + 130.0f)) {
                    temp2 = 1;
                    doorObj->segment.trans.y_position += 2.0f * updateRateF;
                }
    ...
    if ((twoPlayerAdv && obj->properties.npc.action != SILVER_COIN_INACTIVE) || (!twoPlayerAdv && obj->properties.npc.action == SILVER_COIN_ACTIVE)) {
        ...
        if (interactObj->distance < 80) {
            ...
            if (racerObj != NULL && racerObj->segment.header->behaviorId == BHV_RACER) {
                racer = (Object_Racer *) racerObj->unk64;
                if (racer->playerIndex != PLAYER_COMPUTER) {
                    if (racer->raceFinished == FALSE && !(obj->properties.npc.action & (SILVER_COIN_COLLECTED << racer->playerIndex))) {
...
void obj_loop_weaponballoon(Object *weaponBalloonObj, s32 updateRate) {
    ...
    weaponBalloonObj->segment.trans.scale = weaponBalloon->radius * (1.0f - (weaponBalloon->unk4 / 90.0f));
    if (weaponBalloonObj->segment.trans.scale < 0.001f) {
        weaponBalloonObj->segment.trans.scale = 0.001f;
    }
    if (weaponBalloonObj->segment.trans.scale < 0.1f) {
...
void obj_init_weapon(Object *obj, UNUSED LevelObjectEntry_Weapon *entry) {
    ...
    obj->interactObj->pushForce = 0;
    obj->properties.projectile.timer = 480;
...
void handle_rocket_projectile(Object *obj, s32 updateRate) {
    ...
    if (weapon->weaponID == WEAPON_ROCKET_HOMING) {
        ...
        if (numCheckpoints > 0) {
            if (func_800185E4(weapon->checkpoint, obj, posX, posY, posZ, &weapon->checkpointDist, (u8 *) &surface) == FALSE) {
                weapon->checkpoint++;
    ...
    if (obj->interactObj->obj != NULL) {
        ...
        if (interactObj == weapon->owner) {
            if (obj->properties.projectile.timer < 450 && obj->properties.projectile.timer) {
                goto block_25;
            }
            goto block_37;
        }
...
void func_8003F2E8(Object *weaponObj, s32 updateRate) {
    ...
    if (weaponProperties->unk4 < 2) {
        ...
        if (weaponOwner->vehicleID != VEHICLE_PLANE) {
            var_a1 = 34;
            weaponObj->segment.y_velocity -= 2.0f;
            weaponObj->segment.x_velocity -= (weaponObj->segment.x_velocity / 8.0f);
            weaponObj->segment.z_velocity -= (weaponObj->segment.z_velocity / 8.0f);
        } else {
...
void obj_init_buoy_pirateship(Object *obj, UNUSED LevelObjectEntry_Buoy_PirateShip *entry, UNUSED s32 arg2) {
    obj->unk64 = func_800BE654(obj->segment.object.segmentID, obj->segment.trans.x_position, obj->segment.trans.z_position);
    obj->interactObj->flags = INTERACT_FLAGS_SOLID;
    obj->interactObj->unk11 = 0;
    obj->interactObj->hitboxRadius = 30;
    obj->interactObj->pushForce = 0;
...
void obj_loop_log(Object *obj, s32 updateRate) {
    ...
    if (obj->unk5C->unk100 != NULL) {
        ...
        diffX *= diffZ;
        obj->properties.log.angleVel -= (s32) (diffX / 4.0f);
    ...
    obj->segment.trans.y_position -= (f32) obj->properties.log.velocityY * 1.0f;
...
void obj_loop_frog(Object *obj, s32 updateRate) {
    ...
    case FROG_HOP:
        ...
        if (func_8002BAB0(obj->segment.object.segmentID, obj->segment.trans.x_position, obj->segment.trans.z_position, &sp6C) != 0) {
            obj->segment.trans.y_position = 0.0f;
            ignore_bounds_check();
