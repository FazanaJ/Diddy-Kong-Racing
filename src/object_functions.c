...
void obj_loop_laserbolt(Object *obj, s32 updateRate) {
    ...
    if (hasCollision) {
        obj_spawn_effect(obj->segment.trans.x_position, obj->segment.trans.y_position - 36.0f,
                         obj->segment.trans.z_position, ASSET_OBJECT_ID_BOMBEXPLOSION, SOUND_NONE, 0.2f, 0);
        delete = TRUE;
    }
    ...
    if (obj->interactObj->distance < 80) {
        ...
        if (racerObj && racerObj->behaviorId == BHV_RACER) {
            ...
            delete = TRUE;
            obj_spawn_effect(obj->segment.trans.x_position, obj->segment.trans.y_position - 36.0f,
                             obj->segment.trans.z_position, ASSET_OBJECT_ID_BOMBEXPLOSION, SOUND_EXPLOSION, 0.5f, 0);
        }
    }
    if (delete) {
        free_object(obj);
    ...
    if (obj->interactObj->distance < obj->properties.zipper.radius) {
        ...
        for (i = 0; i < numObjects; i++) {
            ...
            if (racer->boostTimer < 15 && racer->groundedWheels) {
                ...
                if ((s32) ((diffX * diffX) + (diffY * diffY) + (diffZ * diffZ)) <
                    obj->properties.zipper.radius * obj->properties.zipper.radius) {
                    if (racer->playerIndex != PLAYER_COMPUTER) {
...
void obj_loop_parkwarden(Object *obj, s32 updateRate) {
    ...
    var_a2 = FALSE;
    if ((obj->properties.npc.action == NULL) && (distance < 300.0f) &&
...
void obj_loop_door(Object *doorObj, s32 updateRate) {
    ...
    if (door->doorID >= 0) {
        ...
        if (door->doorType & 2) {
            ...
            doorObj->segment.trans.y_rotation -= angleVel * updateRate;
            if (angleVel != 0) {
                playSound = TRUE;
            }
        }
...
void obj_loop_bridge_whaleramp(Object *obj, s32 updateRate) {
    ...
    if (entry->unkB != 3) {
        if (obj->properties.common.unk0 != 0) {
            bobAmount = 2.0f * (f32) entry->bobAmount;
            if (bobAmount > 0.0f) {
                if (obj->segment.trans.y_position < whaleRamp->homeY + bobAmount) {
                    obj->segment.trans.y_position += updateRateF * 2.0f;
                }
            } else {
    ...
    if (properties->status == BANANA_COLLECTED) {
        ...
        if (properties->status == BANANA_DROPPED) {
            ...
            if (banana->droppedVehicleID != VEHICLE_PLANE) {
                obj->segment.y_velocity -= 0.5f * updateRateF;
                obj->segment.x_velocity -= (obj->segment.x_velocity * 0.025f) * updateRateF;
                obj->segment.z_velocity -= (obj->segment.z_velocity * 0.025f) * updateRateF;
            } else {
                obj->segment.x_velocity = 0.0f;
...
void obj_loop_weaponballoon(Object *weaponBalloonObj, s32 updateRate) {
    ...
    weaponBalloonObj->segment.trans.scale = weaponBalloon->scale * (1.0f - (weaponBalloon->respawnTime / 90.0f));
