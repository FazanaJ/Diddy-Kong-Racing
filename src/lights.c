...
void setup_lights(s32 count) {
    ...
    free_lights();
    gMaxLights = count;
    temp_v0 = (ObjectLight **) allocate_from_main_pool_safe(gMaxLights * (sizeof(s32 *) + sizeof(ObjectLight) + sizeof(unk800DC960) + sizeof(Vec3f)), MEMP_LIGHTS);
    newCount = gMaxLights;
    gActiveLights = temp_v0;
    D_800DC954 = (ObjectLight *) (newCount + (temp_v0));
    D_800DC960 = (unk800DC960 *) (newCount + (D_800DC954));
    D_800DC964 = (Vec3f *) (newCount + (D_800DC960));
...
f32 light_distance_calc(ObjectLight *light) {
    ...
    switch (light->type) {
        case 1:
            mag = 1.0f - ((gLightDistance) * (light->radiusMag * light->radiusMag));
            dist *= mag;
            break;
        case 2:
            mag = 1.0f - sqrtf((gLightDistance) * (light->radiusMag * light->radiusMag));
            dist *= mag;
            break;
        case 3:
            mag = coss_f((gLightDistance) * (light->radiusMag * light->radiusMag) * (16384.0f));
            dist *= mag;
            break;
        case 4:
            mag = coss_f((gLightDistance) * (light->radiusMag * light->radiusMag) * (16384.0f));
