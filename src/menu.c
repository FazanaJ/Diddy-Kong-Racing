...
void savemenu_render(UNUSED s32 updateRate) {
    ...
    if (drawLowerElements) {
        ...
        for (scroll = 0; scroll < 2; scroll += 1, temp += 16) {
            render_textured_rectangle(&sMenuCurrDisplayList, gMenuSelectionArrowDown, (gScreenWidth / 2), temp, 255, 255,
                                      255, 255);
        }
    }
...
s32 menu_boot_loop(s32 updateRate) {
    ...
    if (y < 300) {
        render_textured_rectangle(&sMenuCurrDisplayList, sGameTitleTileOffsets, (gScreenWidth / 2), y, 255, 255, 255,
                                  255);
        reset_render_settings(&sMenuCurrDisplayList);
        set_text_font(FONT_SMALL);
        if (gExpansionPak) {
...
void bootscreen_init_cpak(void) {
    ...
    // Starting point
    gBootPakData[0] = allocate_from_main_pool_safe(SAVE_SIZE, MEMP_MENU);
    ...
    if (sControllerPakError != PAK_ERROR_NONE) {
        ...
        if (gOpacityDecayTimer < (16 - sControllerPakMenuNumberOfRows)) {
            if ((gOptionBlinkTimer & 8) != 0) {
                render_textured_rectangle(&sMenuCurrDisplayList, gMenuSelectionArrowDown, (gScreenWidth / 2), yPos + 8,
                                          255, 255, 255, 255);
                reset_render_settings(&sMenuCurrDisplayList);
            }
        } else {
            ...
            if (gMenuStage != TRACKMENU_MIRROR) {
                if (gNumberOfActivePlayers == 1) {
                    ...
                    if (gMenuStage <= TRACKMENU_OPT_1) {
                        render_textured_rectangle(&sMenuCurrDisplayList, gRaceSelectionVehicleTitleTexture, 136 + ((gScreenWidth - 320) / 2),
                                                  regionOffset + 114, 255, 255, 255, sMenuGuiOpacity);
                    } else {
                        render_textured_rectangle(&sMenuCurrDisplayList, gRaceSelectionTTTitleTexture, 136 + ((gScreenWidth - 320) / 2),
                                                  regionOffset + 114, 255, 255, 255, sMenuGuiOpacity);
                        for (k = 0, y = regionOffset + 151; k < 2; k++, y += 24) {
                            if (k == gTracksMenuTimeTrialHighlightIndex) {
                                render_textured_rectangle(&sMenuCurrDisplayList, gTrackSelectTTImage[k * 3 + 1], 104 + ((gScreenWidth - 320) / 2), y,
                                                          255, 255, 255, sMenuGuiOpacity);
                            } else {
                                render_textured_rectangle(&sMenuCurrDisplayList, gTrackSelectTTImage[k * 3 + 2], 104 + ((gScreenWidth - 320) / 2), y,
                                                          255, 255, 255, sMenuGuiOpacity);
                            }
                        }
                    }
...
s32 menu_cinematic_loop(UNUSED s32 updateRate) {
    ...
    if (gCinematicPortraits != NULL) {
        for (i = 0; gCinematicPortraits[i] != -1; i++) {
            render_textured_rectangle(&sMenuCurrDisplayList, gRacerPortraits[gCinematicPortraits[i]], 24, 16 + (44 * i),
                                      255, 255, 255, 255);
        }
    }
    gIgnorePlayerInputTime = 0;
...
void menu_image_load(s32 imageID) {
    if (gMenuImages == NULL) {
        gMenuImages = allocate_from_main_pool_safe(sizeof(MenuAsset) * 18, MEMP_MENU);
    }
