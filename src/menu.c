...
void menu_init(u32 menuId) {
    ...
    for (i = 0; i < ARRAY_COUNT(D_80126808); i++) {
        D_80126808[i] = 0;
    }
    music_stop();
...
void func_80085B9C(UNUSED s32 updateRate) {
    ...
    if (drawTexturedRectangle) {
        ...
        for (var_s2 = 0; var_s2 < 2; var_s2 += 1, temp += 16) {
            render_textured_rectangle(&sMenuCurrDisplayList, gMenuSelectionArrowDown, (gScreenWidth / 2), temp, 255,
                                      255, 255, 255);
        }
    }
    ...
    if (var_s3) {
        ...
        var_s1 = sp5C + ((gScreenWidth - 320) / 2);
    ...
    set_text_colour(255, 255, 255, 0, 255);
    if (drawOk) {
        draw_text(&sMenuCurrDisplayList, gScreenWidth / 2, 128, (char *) D_800E8208, ALIGN_MIDDLE_CENTER); // "OK?"
    }
    if (drawPleaseWait) {
...
s32 menu_boot_loop(s32 updateRate) {
    ...
    if (temp < 300) {
        render_textured_rectangle(&sMenuCurrDisplayList, sGameTitleTileOffsets, (gScreenWidth / 2), temp, 255, 255, 255,
                                  255);
        reset_render_settings(&sMenuCurrDisplayList);
        set_text_font(FONT_SMALL);
        if (gExpansionPak) {
            draw_text(&sMenuCurrDisplayList, (gScreenWidth / 2), SCREEN_HEIGHT - 40, "Expansion Pak Detected",
                      ALIGN_MIDDLE_CENTER);
        } else {
            draw_text(&sMenuCurrDisplayList, (gScreenWidth / 2), SCREEN_HEIGHT - 40, "Expansion Pak Missing",
                      ALIGN_MIDDLE_CENTER);
        }
    }
...
void render_magic_codes_ui(UNUSED s32 updateRate) {
    ...
    while (gMagicCodeMenuStrings[i] != NULL) {
        ...
        set_text_colour(255, 255, 255, alpha, 255);
        draw_text(&sMenuCurrDisplayList, (gScreenWidth / 2), temp, gMagicCodeMenuStrings[i], ALIGN_MIDDLE_CENTER);
        i++;
        temp += yPos;
    }
    set_text_font(ASSET_FONTS_FUNFONT);
    ...
    if (gOptionsMenuItemIndex == 4) {
        ...
        draw_text(&sMenuCurrDisplayList, (gScreenWidth / 2), 144, D_80126C58, ALIGN_MIDDLE_CENTER);
...
void render_magic_codes_list_menu_text(UNUSED s32 updateRate) {
    ...
    for (i = gMenuOptionCount, cheatData = (*gCheatsAssetData) + 1;
        ...
        draw_text(&sMenuCurrDisplayList, 48 + ((gScreenWidth - 320) / 2), yPos, (char *) (*gCheatsAssetData) + cheatData[(D_80126C80[i] << 1) + 1],
                  ALIGN_TOP_LEFT);
...
void draw_character_select_text(UNUSED s32 arg0) {
    ...
    if (gMenuDelay >= -0x16 && gMenuDelay < 0x17) {
        ...
        if (gNumberOfReadyPlayers == gNumberOfActivePlayers && gNumberOfActivePlayers > 0) {
            yPos = 208;
            draw_text(&sMenuCurrDisplayList, (gScreenWidth / 2), yPos, (char *) D_800E8230 /* "OK?" */,
                      ALIGN_MIDDLE_CENTER);
        }
        reset_render_settings(&sMenuCurrDisplayList);
...
void render_file_select_menu(UNUSED s32 updateRate) {
    ...
    for (i = 0; i < NUMBER_OF_SAVE_FILES; i++) {
        ...
        func_80080580(NULL, x, (gScreenHeight / 2) - gFileSelectButtons.y, gFileSelectButtons.width,
                      ...
                      colour, gMenuObjects[TEXTURE_SURFACE_BUTTON_WOOD]);
        x += 92;
    }
    func_80080BC8(&sMenuCurrDisplayList);
    if (gOpacityDecayTimer == 0) {
        ...
        for (i = 0; i < NUMBER_OF_SAVE_FILES; i++) {
            ...
            if (gSavefileInfo[saveNum].isStarted) {
                ...
                render_menu_image(var_s2, x + gFileSelectElementPos[2], gFileSelectButtons.y + gFileSelectElementPos[3],
                                  0, 0, 0, 128);
                sprite_anim_off(TRUE);
                gMenuImageStack->unk18 = gSavefileInfo[saveNum].balloonCount / 10;
                render_menu_image(0, x + gFileSelectElementPos[6] - 6, gFileSelectButtons.y + gFileSelectElementPos[7],
                                  0, 0, 0, 128);
                gMenuImageStack->unk18 = gSavefileInfo[saveNum].balloonCount % 10;
                render_menu_image(0, x + gFileSelectElementPos[6] + 6, gFileSelectButtons.y + gFileSelectElementPos[7],
                                  0, 0, 0, 128);
                ...
                sMenuGuiColourG = 64;
                sMenuGuiColourB = 64;
                render_menu_image(10, x + gFileSelectElementPos[8], gFileSelectButtons.y + gFileSelectElementPos[9], 0,
                                  0, 0, 128);
                sMenuGuiColourG = 255;
                sMenuGuiColourB = 255;
                sprite_opaque(TRUE);
    ...
    if (D_80126484) {
        if (D_80126494 == 0) {
            ...
            draw_text(&sMenuCurrDisplayList, (gScreenWidth / 2), yPos, (char *) &D_800E8234 /* "OK?" */,
                      ALIGN_MIDDLE_CENTER);
        }
    } else if (D_80126488) {
        if (D_80126494 == 0) {
            ...
            draw_text(&sMenuCurrDisplayList, (gScreenWidth / 2), yPos, (char *) &D_800E8238 /* "OK?" */,
                      ALIGN_MIDDLE_CENTER);
        }
    } else if (!D_80126CC0) {
        if (gMenuOptionCount == 1) {
...
void render_track_select_setup_ui(UNUSED s32 updateRate) {
    ...
    if (gMenuDelay >= -22 && gMenuDelay <= 30) {
        ...
        if (D_801269C8 < 4) {
            ...
            show_timestamp(settings->courseTimesPtr[gPlayerSelectVehicle[PLAYER_ONE]][gTrackIdForPreview], 22 + ((gScreenWidth - 320) / 2), 53, 128,
                           255, 255, FONT_COLOURFUL);
            show_timestamp(settings->flapTimesPtr[gPlayerSelectVehicle[PLAYER_ONE]][gTrackIdForPreview], 22 + ((gScreenWidth - 320) / 2), 33, 255,
        ...
        if (D_801269C8 != 5) {
            if ((gMenuOptionCount == 2 && !sp74) || (gMenuOptionCount == 3 && sp74) || gMenuOptionCount == 4) {
                ...
                draw_text(&sMenuCurrDisplayList, gScreenWidth / 2, regionOffset + 172, (char *) D_800E823C /* "OK?" */,
                          ALIGN_MIDDLE_CENTER);
            }
        }
        sMenuGuiOpacity = 255;
            ...
            if (!(get_map_race_type(sp58) & 0x40)) {
                if (arg2 == 0) {
                    ...
                    if (gMenuOptionCount != 0) {
                        ...
                        set_text_background_colour(0, 0, 0, 0);
                        set_text_colour(255, 255, 255, 0, 255);
                        draw_text(&sMenuCurrDisplayList, (gScreenWidth / 2), yOffset + 172, (char *) D_800E8240,
                                  ALIGN_MIDDLE_CENTER);
                    }
                } else {
                    set_text_font(FONT_LARGE);
