...
void weather_reset(s32 weatherType, s32 density, s32 velX, s32 velY, s32 velZ, s32 intensity, s32 opacity) {
    ...
    for (j = 0; j < playerCount; j++) {
        ...
        for (i = 0; i < gSnowDensity[j]; i++) {
            gSnowPhysics[j][i].x_position = rand_range(0, gSnowGfx.radiusX);
            gSnowPhysics[j][i].y_position = rand_range(0, gSnowGfx.radiusY);
            gSnowPhysics[j][i].z_position = rand_range(0, gSnowGfx.radiusZ);
            gSnowPhysics[j][i].index = rand_range(0, gSnowGfx.size - 1);
        }
    }
...
void snow_init(void) {
    ...
    for (i = 0; i < gSnowGfx.size; i++) {
        gSnowGfx.pos[i].x = coss_f(offset & 0xFFFF) * 0x80000;
        gSnowGfx.pos[i].y = 0xFFFC0000;
        gSnowGfx.pos[i].z = sins_f(offset & 0xFFFF) * 0x20000;
        offset += step;
    }
...
void snow_vertices(s32 index) {
    ...
    for (i = 0; i < gSnowParticleCount; i++) {
        posF.x = (((gSnowPhysics[index][i].x_position - camX) & gSnowGfx.radiusX) + gSnowGfx.offsetX) * (1.0f / 65536.0f);
        posF.y = (((gSnowPhysics[index][i].y_position - camy) & gSnowGfx.radiusY) + gSnowGfx.offsetY) * (1.0f / 65536.0f);
        posF.z = (((gSnowPhysics[index][i].z_position - camz) & gSnowGfx.radiusZ) + gSnowGfx.offsetZ) * (1.0f / 65536.0f);
...
void snow_render(UNUSED s32 index) {
    ...
    if (gSnowGfx.texture != NULL) {
        ...
        if (gSnowVertCount >= 4) {
            ...
            gSPMatrixDKR(gCurrWeatherDisplayList++, OS_PHYSICAL_TO_K0(mtx), G_MTX_DKR_INDEX_0);
