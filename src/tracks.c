...
void aa_manage(s32 mode) {
    ...
    if (gScenePlayerViewports == ONE_PLAYER) {
        ...
        if (gConfig.multiAA >= cam_get_viewport_layout()) {
            aaMode = gConfig.antiAliasing;
        } else {
            aaMode = AA_OFF;
        }
...
void render_scene(Gfx **dList, Mtx **mtx, Vertex **vtx, Triangle **tris, s32 updateRate) {
    ...
    for (j = gSceneCurrentPlayerID = 0; j < numViewports; gSceneCurrentPlayerID++, j = gSceneCurrentPlayerID) {
        ...
        if (gCurrentLevelHeader2->weatherEnable > 0 && (numViewports < 2 || gConfig.multiWeather)) {
            weather_update(&gTrackDL, &gTrackMtxPtr, &gTrackVtxPtr, &gTrackTriPtr,
                           tempUpdateRate);
        }
...
void render_level_geometry_and_objects(void) {
    ...
    sortbuffer_pop(&gTrackDL, SORT_OPA);
    sortbuffer_pop(&gTrackDL, SORT_DECAL);
    ...
    sortbuffer_pop(&gTrackDL, SORT_OPA);
    sortbuffer_pop(&gTrackDL, SORT_DECAL);
    sortbuffer_pop(&gTrackDL, SORT_XLU);
