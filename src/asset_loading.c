...
u32 *asset_table_load(u32 assetIndex) {
    ...
    out = (u32 *) mempool_alloc_safe(size, PP_RAM_ASSETTABLE);
    if (out == 0) {
        return NULL;
    }
...
void assettable_seek_s32(s32 modelID, s32 *offset, s32 *size, s32 assetIndex) {
    s32 table[2];
    asset_load(assetIndex, (u32) &table, modelID * sizeof(s32), 2 * (sizeof(s32)));
    *offset = table[0];
...
void assettable_seek_s16(s32 modelID, s32 *offset, s32 *size, s32 assetIndex) {
    s16 table[4];
    asset_load(assetIndex, (u32) &table, modelID * (sizeof(s16)), 2 * (sizeof(s16)));
    *offset = table[0];
    *size = table[1];
}