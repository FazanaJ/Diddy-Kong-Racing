...
f32 gParticleGravityTable[8] = { 0.0f, 0.1f, 0.2f, 0.3f, 0.45f, 0.525f, 0.6f, 0.8f };
...
void func_800AF29C(Particle *arg0, s32 behaviourID, s32 propertyID, s16 velX, s16 velY, s16 velZ) {
    ...
    if (flags & 0x4000) {
        ...
        arg0->data.unkC_400.unkC =
            (s32 *) mempool_alloc_safe(arg0->data.lifeTime * sizeof(uintptr_t), MEMP_PARTICLES);
...
void func_800B0010(Particle *arg0, Particle *arg1, Particle *arg2, ParticleBehavior *arg3) {
    ...
    if (flags) {
        if (flags & PARTICLE_VEL_X) {
            arg0->segment.x_velocity +=
                (f32) get_random_number_from_range(-arg3->velocityRangeX1, arg3->velocityRangeX1) * 0.00001525878906f;
        }
        if (flags & PARTICLE_VEL_Y) {
            arg0->segment.y_velocity +=
                (f32) get_random_number_from_range(-arg3->velocityRangeY1, arg3->velocityRangeY1) * 0.00001525878906f;
        }
        if (flags & PARTICLE_VEL_Z) {
            arg0->segment.z_velocity +=
                (f32) get_random_number_from_range(-arg3->velocityRangeZ1, arg3->velocityRangeZ1) * 0.00001525878906f;
        }
    }
    ...
    if (arg3->flags & 4) {
        ...
        if (flags & PARTICLE_UNK00000010) {
            velocityPos.z +=
                (f32) get_random_number_from_range(-arg3->gravityRange2, arg3->gravityRange2) * 0.00001525878906f;
        }
...
void func_800B03C0(Particle *particle, Particle *arg1, Particle *arg2, ParticleBehavior *behaviour) {
    ...
    if (behaviour->behaviourFlags & PARTICLE_FORWARDVEL) {
        particle->forwardVel +=
            ...
            0.00001525878906f; // 0.00001525878906 ~= 1.0/65536.0
    }
    if (behaviour->flags & 1) {
        posVel.x = 0.0f;
        posVel.y = 0.0f;
...
Particle *func_800B1130(Particle *arg0, Particle *arg1) {
    ...
    if (behaviour->behaviourFlags & PARTICLE_UNK00020000) {
        sp24 += (f32) get_random_number_from_range(-behaviour->unk8C, behaviour->unk8C) * 0.00001525878906f;
    }
    var_v0->segment.trans.scale = properties->scale * sp24;
    sp24 = behaviour->unk54;
    if (behaviour->behaviourFlags & PARTICLE_UNK00040000) {
        sp24 += (f32) get_random_number_from_range(-behaviour->unk90, behaviour->unk90) * 0.00001525878906f;
    }
...
void move_particle_basic_parent(Particle *particle, s32 updateRate) {
    ...
    particle->segment.trans.rotation.y_rotation += particle->angleVelY * updateRate;
    particle->segment.trans.rotation.x_rotation += particle->angleVelX * updateRate;
    particle->segment.trans.rotation.z_rotation += particle->angleVelZ * updateRate;
