...
// USB Memory location
#define DEFAULT_DEBUG_ADDRESS \
    (0x04000000 - DEBUG_ADDRESS_SIZE) // Put the debug area at the 64MB - DEBUG_ADDRESS_SIZE area in ROM space
...
static u8 usb_buffer_align[BUFFER_SIZE +
                           16]; // IDO doesn't support GCC's __attribute__((aligned(x))), so this is a workaround
...
// Before we do anything, check that we are using an emulator
    ...
    // It's unlikely that emulators handle this, so we'll write to the VI_TEST_ADDR register and readback 0x40 bytes
    // from its address If they don't match, we probably have an emulator
    buff = (*(u32 *) 0xA4400038);
...
static void usb_64drive_cui_write(u8 datatype, u32 offset, u32 size) {
    ...
    usb_io_write(D64_REG_USBP1R1,
                 USBHEADER_CREATE(datatype, ALIGN(size, 4))); // Align size to 32-bits due to bugs in the firmware
...
static void usb_everdrive_readusb(s8 *buffer, int size) {
    u16 block, addr;
