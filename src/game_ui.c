...
void init_hud(UNUSED s32 viewportCount) {
    ...
    gAssetHudElements = allocate_from_main_pool_safe(gAssetHudElementIdsCount * (sizeof(void *) + 1), MEMP_HUD);
    ...
    gPlayerHud[PLAYER_ONE] = allocate_from_main_pool_safe(playerCount * sizeof(HudData), MEMP_HUD);
...
void render_hud(Gfx **dList, MatrixS **mtx, Vertex **vertexList, Object *arg3, s32 updateRate) {
    ...
    gHudCurrentViewport = gActiveCameraID;
    if (D_8012718A) {
        ...
        if (get_game_mode() != GAMEMODE_MENU) {
            ...
            if (gShowHUD == 0) {
                ...
                if (check_if_showing_cutscene_camera() == FALSE && gHudRaceStart == FALSE &&
                    ...
                    if (D_80126D35 != 0) {
                        ...
                        if (gHudBounceTimer >= 0x8000) {
                            ...
                            gHudBounceMag = gHudBounceMag / 2.0f;
                            if (gHudBounceMag <= 0.125f) {
                                gHudRaceStart = TRUE;
                                gHudOffsetX = 0;
                            }
                        }
                ...
                if (racer->raceFinished == TRUE) {
                    ...
                    } else if (gNumberOfViewports == VIEWPORTS_COUNT_1_PLAYER && racer->unk1AC == 1) {
    ...
    if (gShowCourseDirections) {
        ...
        if (timer > 0) {
            ...
            if (type) {
                ...
                if (gHUDNumPlayers == ONE_PLAYER && racer->raceFinished == FALSE && racer->indicator_type &&
                    ...
                    indicator->x = -indicator->x + (gScreenWidth - 320);
                    func_800AA600(&gHUDCurrDisplayList, &gHUDCurrMatrix, &gHUDCurrVertex, indicator);
                    indicator->x = -indicator->x + (gScreenWidth - 320);
        ...
        if (D_800E27B8) {
            if (D_800E27B8 & 0x20) {
                ...
                func_800AA600(&gHUDCurrDisplayList, &gHUDCurrMatrix, &gHUDCurrVertex, indicator);
                indicator->x = (f32) -indicator->x + (gScreenWidth - 320);
                func_800AA600(&gHUDCurrDisplayList, &gHUDCurrMatrix, &gHUDCurrVertex, indicator);
                indicator->x = (f32) -indicator->x + (gScreenWidth - 320);
                gDPSetPrimColor(gHUDCurrDisplayList++, 0, 0, 255, 255, 255, 255);
            }
...
void func_800A14F0(Object *racerObj, s32 updateRate) {
    ...
    if (numRacers == 4) {
        func_80068508(TRUE);
        sp44 = gCurrentHud->challengePortrait.x;
        sp3C = gCurrentHud->challengePortrait.y;
        sp40 = (gCurrentHud->eggChallengeIcon.x - sp44);
        sp38 = (gCurrentHud->eggChallengeIcon.y - sp3C);
        for (i = 0; i < 4; i++) {
            curRacer = &racers[i]->unk64->racer;
            ...
            if (gNumActivePlayers == 1) {
                ...
                gCurrentHud->challengePortrait.x = sp44;
                if (osTvType == 0) {
                    gCurrentHud->challengePortrait.y += 66.0f;
                    gCurrentHud->eggChallengeIcon.y += 66.0f;
                } else {
                    gCurrentHud->challengePortrait.y += 55.0f;
                    gCurrentHud->eggChallengeIcon.y += 55.0f;
                }
            }
            gCurrentHud->eggChallengeIcon.x = gCurrentHud->challengePortrait.x + sp40;
        }
        gCurrentHud->challengePortrait.x = sp44;
        gCurrentHud->eggChallengeIcon.x = sp44 + sp40;
        gCurrentHud->challengePortrait.y = sp3C;
        gCurrentHud->eggChallengeIcon.y = sp3C + sp38;
        func_80068508(FALSE);
    }
}
...
void func_800A19A4(Object_Racer *racer, UNUSED s32 updateRate) {
    ...
    if (gCurrentHud->eggChallengeIcon.unk1A < 0) {
        sp44 = (gCurrentHud->eggChallengeIcon.unk1A * 2) + 256;
    } else {
        sp44 = 255 - (gCurrentHud->eggChallengeIcon.unk1A * 2);
    }
    if (sp44 > 254) {
        sp44 = 254;
    }
    ...
    if (gNumActivePlayers == 1) {
        if (!gCutsceneCameraActive) {
            ...
            if (racer->raceFinished == FALSE) {
                ...
                x = gScreenWidth - 64 + gHudBounceX + gHudOffsetX;
...
void render_race_finish_position(Object_Racer *racer, s32 updateRate) {
    ...
    temp_a3 = (unk800A497C *) &gCurrentHud->raceFinishPosition1;
    temp_s0 = (unk800A497C *) &gCurrentHud->raceFinishPosition2;
    ...
    if (drawPosition) {
        ...
        func_800AA600(&gHUDCurrDisplayList, &gHUDCurrMatrix, &gHUDCurrVertex, (HudElement *) temp_a3);
        func_800AA600(&gHUDCurrDisplayList, &gHUDCurrMatrix, &gHUDCurrVertex, (HudElement *) temp_s0);
        gDPSetPrimColor(gHUDCurrDisplayList++, 0, 0, 255, 255, 255, 255);
    }
}
...
void render_race_position(Object_Racer *racer, s32 updateRate) {
    ...
    scale = sins_f(scale);
    placeIndex = place - 1;
    gCurrentHud->racePostion.scale = (gCurrentHud->racePostion.unk1C / 127.0f) + (1.0f + (0.5f * scale));
        ...
        if (gCurrentHud->lapTextLap.unk1A != 0) {
            if (gCurrentHud->lapTextLap.unk1A == 2) {
                if (gCurrentHud->lapTextLap.unk1B == 1) {
                    ...
                    if (gCurrentHud->lapTextLap.x == gCurrentHud->lapTextLap.unk1C &&
                        gCurrentHud->lapTextTwo.x == gCurrentHud->lapTextTwo.unk1C &&
                        gCurrentHud->lapTextLap.unk1D == 0) {
                        if (gHUDVoiceSoundMask == NULL) {
    ...
    if (racer->raceFinished == FALSE) {
        ...
        if (racer->balloon_quantity > 0) {
            if (gCurrentHud->weaponDisplay.unk1B < 16 && racer->unk170 == 0) {
                ...
                gCurrentHud->weaponDisplay.scale = (gCurrentHud->weaponDisplay.unk1B * 0.04687f) + 0.25f;
            } else {
            ...
            if (gCurrentHud->weaponDisplay.unk1B > 16) {
                ...
                if (gCurrentHud->weaponDisplay.unk1C > 120) {
                    ...
                    gCurrentHud->weaponDisplay.scale +=
                        0.18f * sins_f(((f32) gCurrentHud->weaponDisplay.unk1C * 682.6583f * 4.0f));
                }
            }
            if (gHUDNumPlayers > ONE_PLAYER) {
                gCurrentHud->weaponDisplay.scale *= 0.75f;
            }
            if (gCurrentHud->weaponDisplay.scale != 1.0f) {
                sprite_opaque(TRUE);
            }
            ...
            if (gCurrentHud->weaponDisplay.unk1B > 0) {
                ...
                if (gHUDNumPlayers != ONE_PLAYER) {
                    gCurrentHud->weaponDisplay.scale /= 2.0f;
                }
                func_800AA600(&gHUDCurrDisplayList, &gHUDCurrMatrix, &gHUDCurrVertex, &gCurrentHud->weaponDisplay);
            }
        }
    ...
    if (gHudLevelHeader->race_type == RACETYPE_CHALLENGE_EGGS) {
        if (gNumActivePlayers == 2 && gHudToggleSettings[gHUDNumPlayers] == 0) {
            ...
            if (gNumActivePlayers == 3) {
                ...
                if (tempVar4 != NULL) {
                    temp_s0 = gCurrentHud->challengePortrait.x;
                    ...
                    if (osTvType == TV_TYPE_PAL) {
                        ...
                        gCurrentHud->challengePortrait.y *= 1.1f;
                        gCurrentHud->eggChallengeIcon.y *= 1.1f;
                    }
                    func_800A19A4(tempVar4, updateRate);
                    gHudColour = COLOUR_RGBA32(255, 255, 255, 254);
                    gCurrentHud->challengePortrait.x = temp_s0;
        ...
        if (gNumActivePlayers == 2 && gHudToggleSettings[gHUDNumPlayers] == 0) {
            ...
            for (sp144 = 0; sp144 < racerCount; sp144++) {
                ...
                if (osTvType == TV_TYPE_PAL) {
                    gCurrentHud->challengePortrait.y += 66.0f;
                    gCurrentHud->treasureMetre.y += 66.0f;
                } else {
                    gCurrentHud->challengePortrait.y += 55.0f;
            ...
            if (tempVar4 != NULL) {
                ...
                if (osTvType == TV_TYPE_PAL) {
                    ...
                    gCurrentHud->challengePortrait.y *= 1.1f;
                    gCurrentHud->treasureMetre.y *= 1.1f;
        ...
        test:
            ...
            if (is_in_time_trial() && func_8001B288()) {
                ...
                if (temp_v0_8 != NULL) {
                    ...
                    tempVar1 = (opacity * (f32) temp_v0_8->segment.object.opacity) * (1.0f / 128.0f);
