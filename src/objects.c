...
void func_8000B020(s32 numberOfVertices, s32 numberOfTriangles) {
    ...
    D_800DC754[0] =
        (s32) mempool_alloc_safe(((numberOfTriangles * 16) + (numberOfVertices * 10)) * 2, MEMP_MISC);
...
void allocate_object_pools(void) {
    ...
    gParticlePtrList = mempool_alloc_safe(sizeof(uintptr_t) * 200, MEMP_PARTICLES);
    D_8011AE6C = mempool_alloc_safe(sizeof(uintptr_t) * 20, MEMP_OBJECTS);
    D_8011AE74 = mempool_alloc_safe(sizeof(uintptr_t) * 128, MEMP_OBJECTS);
    gTrackCheckpoints = mempool_alloc_safe(sizeof(CheckpointNode) * MAX_CHECKPOINTS, MEMP_OBJECTS);
    gCameraObjList = mempool_alloc_safe(sizeof(uintptr_t *) * 20, MEMP_OBJECTS);
    gRacers = mempool_alloc_safe(sizeof(uintptr_t) * 10, MEMP_OBJECTS);
    gRacersByPort = mempool_alloc_safe(sizeof(uintptr_t) * 10, MEMP_OBJECTS);
    gRacersByPosition = mempool_alloc_safe(sizeof(uintptr_t) * 10, MEMP_OBJECTS);
    gAINodes = mempool_alloc_safe(sizeof(uintptr_t) * AINODE_COUNT, MEMP_OBJECTS);
    D_8011ADCC = mempool_alloc_safe(8, MEMP_OBJECTS);
    D_8011AFF4 = mempool_alloc_safe(sizeof(unk800179D0) * 16, MEMP_OBJECTS);
    ...
    gSpawnObjectHeap = mempool_alloc_safe(sizeof(uintptr_t) * 512, MEMP_OBJECTS);
    ...
    gAssetsObjectHeadersTableLength--;
    gLoadedObjectHeaders = mempool_alloc_safe(gAssetsObjectHeadersTableLength * 4, MEMP_HEADERS);
    gObjectHeaderReferences = mempool_alloc_safe(gAssetsObjectHeadersTableLength, MEMP_HEADERS);
    ...
    decrypt_magic_codes(
        ...
        (gAssetsMiscTable[ASSET_MISC_TITLE_SCREEN_DEMO_IDS] - gAssetsMiscTable[ASSET_MISC_MAGIC_CODES]) *
            sizeof(s32 *));
    gObjPtrList = mempool_alloc_safe(sizeof(uintptr_t) * OBJECT_SLOT_COUNT, MEMP_OBJECTS);
    gFirstTimeFinish = 0;
    gTimeTrialEnabled = 0;
    gIsTimeTrial = FALSE;
...
void func_8000C8F8(s32 arg0, s32 arg1) {
    ...
    D_8011AD3E = 0;
    mem = mempool_alloc_safe(0x3000, MEMP_OBJECTS);
    D_8011AEB0[arg1] = mem;
...
Object *func_8000FD54(s32 objectHeaderIndex) {
    ...
    object = (Object *) mempool_alloc(objSize, MEMP_OBJECTS);
    if (object == NULL) {
    ...
    for (i = 0; i < objSize; i++) {
        objectAsRawBytes[i] = 0;
    } // Must be one line! (Why not use bzero?)
 ...
 */
void render_misc_model(Gfx **dList, Object *obj, Vertex *verts, u32 numVertices, Triangle *triangles, u32 numTriangles,
    ...
    switch (obj->behaviorId) {
        case BHV_CHARACTER_FLAG:
            if (obj->properties.characterFlag.characterID >= 0) {
                objData = obj->unk64;
                render_misc_model(dList, obj, objData->character_flag.vertices, 4, objData->character_flag.triangles, 2,
        ...
        case BHV_BUTTERFLY:
            objData = obj->unk64;
            render_misc_model(dList, obj, &objData->butterfly.vertices[objData->butterfly.unkFC * 6], 6,
        ...
        case BHV_FISH:
            ...
            scale *= 0.01f;
            render_misc_model(dList, obj, &objData->fish.vertices[objData->fish.unkFC * 6], 6, objData->fish.triangles,
                              8, objData->fish.texture, RENDER_Z_COMPARE | RENDER_FOG_ACTIVE | RENDER_CUTOUT, 0, scale);
            break;
        case BHV_BOOST:
...
void render_3d_billboard(Gfx **dList, Object *obj) {
    ...
    if (bubbleTrap != NULL || !(obj->behaviorId != BHV_WEAPON || obj->unk64->weapon.weaponID != WEAPON_BUBBLE_TRAP)) {
        ...
        render_bubble_trap(dList, &bubbleTrap->segment.trans, gfxData, (Object *) &objTrans,
                           RENDER_Z_COMPARE | RENDER_SEMI_TRANSPARENT | RENDER_Z_UPDATE);
    } else {
        render_sprite_billboard(dList, &gObjectCurrMatrix, &gObjectCurrVertexList, obj, (unk80068514_arg4 *) gfxData,
                                flags);
    }
    if (hasPrimCol) {
        gDPSetPrimColor((*dList)++, 0, 0, 255, 255, 255, 255);
...
void render_3d_model(Gfx **dList, Object *obj) {
    ...
    if (obj68 != NULL) {
        ...
        if (alpha < 255) {
            meshBatch = render_mesh(dList, objModel, obj, 0, RENDER_SEMI_TRANSPARENT, spB0);
        } else {
            meshBatch = render_mesh(dList, objModel, obj, 0, RENDER_NONE, spB0);
        }
        if (obj->segment.header->unk71) {
            if (hasOpacity) {
                gDPSetPrimColor((*dList)++, 0, 0, intensity, intensity, intensity, alpha);
                    ...
                    if (index >= 0 && index < objModel->unk18) {
                        ...
                        if (1) {
                            ...
                            loopObj->properties.common.unk0 =
                                render_sprite_billboard(dList, &gObjectCurrMatrix, &gObjectCurrVertexList, loopObj,
        ...
        if (meshBatch != -1) {
            ...
            render_mesh(dList, objModel, obj, meshBatch, RENDER_SEMI_TRANSPARENT, spB0);
            if (obj->segment.header->unk71) {
                disable_primitive_colour();
            }
        }
...
void render_object(Gfx **dList, MatrixS **mtx, Vertex **verts, Object *obj) {
    f32 scale;
    set_crash_object(obj, CRASH_OBJ_RENDER);
...
void render_racer_shield(Gfx **dList, MatrixS **mtx, Vertex **vtxList, Object *obj) {
    ...
    if (racer->shieldTimer > 0 && gShieldEffectObject != NULL) {
        ...
        render_mesh(dList, mdl, gShieldEffectObject, 0, RENDER_SEMI_TRANSPARENT, 0);
...
void render_racer_magnet(Gfx **dList, MatrixS **mtx, Vertex **vtxList, Object *obj) {
    ...
    if (D_8011B078[var_t0].a != 0) {
        if (gMagnetEffectObject != NULL) {
            ...
            gObjectTexAnim = TRUE;
            render_mesh(dList, mdl, gMagnetEffectObject, 0, RENDER_SEMI_TRANSPARENT, 0);
 ...
 */
s32 render_mesh(Gfx **dList, ObjectModel *objModel, Object *obj, s32 startIndex, s32 flags, s32 someBool) {
    s32 i;
    s32 textureIndex;
    s32 triOffset;
...
s32 timetrial_load_staff_ghost(s32 mapId) {
    ...
    gAssetColourTag = MEMP_GHOST_DATA;
...
void run_object_init_func(Object *obj, void *entry, s32 param) {
    ...
    set_crash_object(obj, CRASH_OBJ_SPAWN);
    switch (obj->behaviorId) {
        case BHV_RACER:
