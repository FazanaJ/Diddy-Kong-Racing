...
void track_preallocate_objlists(s32 objMap, s32 collectables) {
    ...
    for (j = 0; j < 2; j++) {
        ...
        assetOffset = objMapTable[mapID[j]];
        assetSize = objMapTable[mapID[j] + 1] - assetOffset;
...
Object *spawn_object(LevelObjectEntryCommon *entry, s32 spawnFlags) {
    ...
    switch (curObj->header->behaviorId) {
        ...
        case BHV_RACER:
            ...
            if ((spawnFlags & OBJECT_SPAWN_NO_LODS) == FALSE) {
                i = 1 + addI;
            }
            break;
...
void obj_update(s32 updateRate) {
    ...
    for (i = 0; i < gCollisionObjectCount; i++) {
        if (gCollisionObjects[i]->lightData) {
            func = (void *) gCollisionObjects[i]->lightData;
            (*func)(gCollisionObjects[i], updateRate);
        }
...
void render_3d_model(Object *obj) {
    ...
    //modInst = obj->modelInstances[obj->modelIndex];
    modInst = obj_model_seek(obj, obj->modelIndex);
    if (modInst != NULL) {
...
s32 checkpoint_is_passed(s32 checkpointIndex, Object *obj, f32 objX, f32 objY, f32 objZ, f32 *checkpointDistance,
    ...
    if (!(*isOnAlternateRoute) && prevCheckpoint->altRouteID == -1 && currentCheckpoint->altRouteID != -1) {
        ...
        if (distX * distX + distY * distY + distZ * distZ < altRouteCheckpoint->checkpointID * altRouteCheckpoint->checkpointID) {
    ...
    length = (distX * distX + distY * distY + distZ * distZ);
    if (length > 0.0) {
        length = sqrtf(length);
        distX *= 1.0f / length;
        distY *= 1.0f / length;
...
s32 ainode_find_next(s32 nodeId, s32 nextNodeId, s32 direction) {
    ...
    if (nodeId < -1 || nodeId >= gAINodeAllocCount) {
        return NODE_NONE;
    }
...
Object *ainode_get(s32 nodeID) {
    if (nodeID >= 0 && nodeID < gAINodeAllocCount) {
