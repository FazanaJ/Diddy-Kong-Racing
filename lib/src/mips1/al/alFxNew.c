...
void alFxNew(ALFx *r, ALSynConfig *c, s16 bus, UNUSED ALHeap *hp) {
    ...
    r->length 	     = param[j++];
    r->delay = mempool_alloc_audio(gAudioPool, sizeof(ALDelay) * r->section_count, MEMP_AUDIO);
    r->base = mempool_alloc_audio(gAudioPool, sizeof(s16) * r->length, MEMP_AUDIO);
    r->input = r->base;
    ...
    #define LENGTH	(d->output - d->input)
            ...
            d->rsval	 = 1.0f;
            d->rsdelta	 = 0.0f;
            d->rs 	 = mempool_alloc_audio(gAudioPool, sizeof(ALResampler), MEMP_AUDIO);
            d->rs->state = mempool_alloc_audio(gAudioPool, sizeof(RESAMPLE_STATE), MEMP_AUDIO);
            d->rs->delta = 0.0f;
            d->rs->first = 1;
        } else {
            d->rs = 0;
        ...
        if (param[j]) {
            d->lp = mempool_alloc_audio(gAudioPool, sizeof(ALLowPass), MEMP_AUDIO);
            d->lp->fstate = mempool_alloc_audio(gAudioPool, sizeof(POLEF_STATE), MEMP_AUDIO);
            d->lp->fc = param[j++];
            init_lpfilter(d->lp);
        } else {
            d->lp = 0;
